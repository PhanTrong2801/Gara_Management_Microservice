...
export default function BillingManagement() {
  ...
  const handlePay = async (invoiceNumber) => {
    if (window.confirm('Xác nhận khách hàng đã thanh toán hóa đơn này?')) {
      try {
        await api.post(`/billing/invoices/${invoiceNumber}/pay`, { paymentMethod: 'CASH' });
              ...
              filteredInvoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 text-center">
                    {inv.status === 'UNPAID' ? (
                      <button 
                        onClick={() => handlePay(inv.invoiceNumber)}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg mx-auto"
                      >
